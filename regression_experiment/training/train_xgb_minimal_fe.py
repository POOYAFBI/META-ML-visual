...
# Columns to exclude from features (EXACT same as Full FE training)
...
# Check for missing values (EXACT same handling as Full FE training)
...
# EXACT same split as Full FE training (test_size=0.2, random_state=42)
...
# Initialize XGBoost with EXACT same parameters as Full FE
model = XGBRegressor(
    n_estimators=100,
    max_depth=6,
    learning_rate=0.1,
...
# Print results
print("\n" + "=" * 70)
print("XGBOOST RESULTS - MINIMAL FE")
print("=" * 70)
...
# Full FE results (from XGB_TRAINING_SUMMARY.md)
full_fe_test_rmse = 23072.84
full_fe_test_r2 = 0.8986
...
if rmse_impact_pct > 5:
    ...
    print(f"   • XGBoost depends on engineered features")
    ...
    print(f"   • XGBoost is robust without engineered features")
...
report_lines = []
report_lines.append("# XGBoost Training Summary - Minimal FE")
report_lines.append("")
...
report_lines.append("- **learning_rate:** 0.1")
...
report_lines.append("")
if rmse_impact_pct > 5:
    report_lines.append("**Finding:** XGBoost shows SIGNIFICANT DEPENDENCE on engineered features.")
    report_lines.append("")
    ...
    report_lines.append("**Finding:** XGBoost shows MODERATE DEPENDENCE on engineered features.")
    report_lines.append("")
    report_lines.append("**Evidence:**")
    report_lines.append(f"- Test RMSE increased by {rmse_impact_pct:.2f}%")
    report_lines.append(f"- Test R² decreased by {abs(r2_impact_pct):.2f}%")
elif rmse_impact_pct < -1:
    report_lines.append("**Finding:** XGBoost IMPROVED without engineered features.")
    report_lines.append("")
    report_lines.append("**Evidence:**")
    ...
    report_lines.append("- Gradient boosting can automatically discover patterns from raw features")
    report_lines.append("- Simplifying features improved generalization")
else:
    report_lines.append("**Finding:** XGBoost is ROBUST without engineered features.")
    report_lines.append("")
    report_lines.append("**Evidence:**")
    ...
    report_lines.append("")
    report_lines.append("**Explanation:**")
    report_lines.append("- Gradient boosting can automatically discover feature interactions")
    report_lines.append("- Sequential error correction learns patterns from raw features")
...
# Save report
with open('XGB_MINIMAL_FE_SUMMARY.md', 'w', encoding='utf-8') as f:
    f.write('\n'.join(report_lines))

print("\n✓ Saved: XGB_MINIMAL_FE_SUMMARY.md")
